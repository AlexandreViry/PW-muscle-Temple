...
if(isset($_SESSION["loggedin"]) && $_SESSION["loggedin"] === true){
    if(isset($_SESSION["tipousuario_id"])) {
...
$email = $password = $confirm_password = $dni = $name = $surname = $phone = "";
$email_err = $password_err = $confirm_password_err = $dni_err = $name_err = $surname_err = $phone_err = "";
...
if($_SERVER["REQUEST_METHOD"] == "POST"){
    ...
    if(empty($email_err) && empty($password_err) && empty($confirm_password_err) && empty($dni_err) && empty($name_err) && empty($surname_err) && empty($phone_err)){
        
        // Prepare an insert statement
        $sql = "INSERT INTO usuario (email, password, dni, nombre, apellidos, telefono) VALUES (?, ?, ?, ?, ?, ?)";

        if($stmt = mysqli_prepare($link, $sql)){
            // Bind variables to the prepared statement as parameters
            mysqli_stmt_bind_param($stmt, "sssssd", $param_email, $param_password, $param_dni, $param_name, $param_surname, $param_phone);
...
<head>
    ...
    <style type="text/css">
        ...
        .left-image { float: left; margin-right: 20px; margin top: 140px; }
